...
from .transformer import build_leaf_encoder as build_leaf_enc
from .transformer import build_leaf_decoder as build_leaf_dec
from .transformer import build_root_transformer as build_root


class Leaf_Transformer(nn.Module):
    """Encoder Part for HiFormer"""
...
class SWiG_Criterion_Leaf(nn.Module):
    """ 
...
class SWiG_Criterion_Root(nn.Module):
    """ 
    Loss for CoFormer with SWiG dataset, and CoFormer evaluation.
    """
    def __init__(self, weight_dict, SWiG_json_train=None, SWiG_json_eval=None, idx_to_role=None):
...
def build_leaf_transformer(args):
    backbone = build_backbone(args)
    transformer1 = build_leaf_enc(args)
    transformer2 = build_leaf_dec(args)

    model = Leaf_Transformer(backbone, transformer1, transformer2,
    ...
    if not args.inference:
        ...
        if not args.test:
            criterion = SWiG_Criterion_Leaf(weight_dict=weight_dict, 
...
def build_root_transformer(args):
    transformer = build_root(args)

    model = Root_Transformer(transformer, args.num_noun_classes, args.vidx_ridx)
    criterion = None
    ...
    if not args.inference:
        ...
        if not args.test:
            criterion = SWiG_Criterion_Root(weight_dict=weight_dict, 
                                      SWiG_json_train=args.SWiG_json_train, 
                                      SWiG_json_eval=args.SWiG_json_dev,
                                      idx_to_role=args.idx_to_role)
        else:
